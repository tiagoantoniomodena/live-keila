...
# ─────────────────────────────────────────────
def _nova_conexao():
    """Abre conexão via Transaction Pooler do Supabase (porta 6543 — funciona no Streamlit Cloud)."""
...
# ─────────────────────────────────────────────
# CRIAÇÃO DAS TABELAS (roda na primeira vez)
# ─────────────────────────────────────────────
def criar_tabelas():
    run("""
...
def sincronizar_clientes():
    ...
    for row in fontes:
        ...
        vistos.add(nome)
        run(
            "INSERT INTO clientes (nome, data_cadastro) VALUES (%s, %s) ON CONFLICT DO NOTHING",
...
def gerar_imagem_cupom(cliente, itens, frete, subtotal, total_geral, data_venda=""):
    ...
    for c in ["C:\\Windows\\Fonts\\cour.ttf", "arial.ttf"]:
        try:
            font_p = ImageFont.truetype(c, 20)
            break
        except Exception:
            continue
    if not font_p:
    ...
    if frete > 0:
        draw.text((m, y), f"FRETE: R$ {frete:.2f}", (0, 0, 0), font=font_p); y += 35
...
def confirmar_finalizar_compra(cliente, telefone, itens, total):
    ...
    if c1.button("✅ Sim, Finalizar", type="primary", use_container_width=True):
        run(
            "INSERT INTO vendas (data, cliente, telefone, itens, frete, total) VALUES (%s,%s,%s,%s,%s,%s)",
...
def confirmar_exclusao(tipo, id_excluir, extra=None):
    ...
    if c1.button("🗑️ Sim, Excluir", type="primary", use_container_width=True):
        if tipo == "venda":
            run("DELETE FROM vendas WHERE id=%s", (id_excluir,))
            ...
            run("DELETE FROM clientes WHERE id=%s", (id_excluir,))
        st.rerun()
    if c2.button("Cancelar", use_container_width=True):
        st.rerun()
...
# ESTADO DA SESSÃO
# ─────────────────────────────────────────────
for k in ["edit_venda_id", "venda_em_edicao", "cupom_aberto", "edit_sacola_item",
          "novo_item_sacola", "cad_editando_id", "cad_cep_dados",
          "hist_itens_uid", "hist_frete", "hist_tel"]:
    if k not in st.session_state:
        st.session_state[k] = None

if not isinstance(st.session_state.hist_itens_uid, dict):
    st.session_state.hist_itens_uid = {}
if not isinstance(st.session_state.hist_frete, dict):
    st.session_state.hist_frete = {}
if not isinstance(st.session_state.hist_tel, dict):
...
st.markdown(
    ...
    '<span style="color:#FFF; font-weight:700;">Live da Keila | Live Sales</span>'
    '</div>',
    unsafe_allow_html=True
)
...
if aba_selecionada == "🛍️ Monitor de Sacolas":
    ...
    # Carrega clientes cadastrados para autocomplete
    todos_clientes = {}
    ...
    with c_grid:
        ...
        for row in sacolas:
            ...
            if busca and busca not in cli_id.lower():
                continue
            if not busca and len(its) == 0:
                continue
            ...
            with st.expander(f"🛍️ {cli_id.upper()}  ·  {qtd_itens} item(ns)  ·  R$ {tot_sac:.2f}"):
                ...
                novo_nome_cli = hi1.text_input(
                    "👤 Nome", value=cli_id,
                    key=f"name_edit_{cli_id}",
                    label_visibility="collapsed",
                    placeholder="Nome do cliente"
                ).strip().lower()
                novo_tel = hi2.text_input(
                    "📞 Tel", value=tel_row,
                    key=f"tel_sac_{cli_id}",
                    label_visibility="collapsed",
                ...
                if hi3.button("💾 Salvar", key=f"save_info_{cli_id}", use_container_width=True):
                    if novo_nome_cli != cli_id:
                        ...
                        run("UPDATE sacolas_ativas SET telefone=%s WHERE cliente=%s", (novo_tel, cli_id))
                    st.rerun()
                ...
                for idx, i in enumerate(its):
                    ...
                    if st.session_state.edit_sacola_item == item_key:
                        ...
                        if ec4.button("💾", key=f"sv_{item_key}", use_container_width=True, help="Salvar"):
                            ...
                            run("UPDATE sacolas_ativas SET itens=%s WHERE cliente=%s", (json.dumps(its), cli_id))
                        ...
                        vc1.markdown(
                            f'<p style="margin:0;padding:6px 0;font-size:0.85rem;font-weight:600;color:#E5E7EB;">{i["nome"]}</p>',
                            unsafe_allow_html=True)
                        vc2.markdown(
...
# ══════════════════════════════════════════════
    ...
    if vendas:
        v_tot = sum(v["total"] for v in vendas)
        f_tot = sum(v["frete"] for v in vendas)
        ...
        with st.container(border=True):
            ...
            if st.session_state.cupom_aberto == vid:
                st.image(gerar_imagem_cupom(
                    v["cliente"], its,
                    v["frete"] or 0, v["total"] or 0, t_v, v["data"]
                ))
            ...
            if st.session_state.edit_venda_id == vid:
                ...
                if st.button("Salvar Edição ✅", key=f"save_{vid}", type="primary", use_container_width=True):
                    ...
                    run(
                        "UPDATE vendas SET itens=%s, frete=%s, total=%s, telefone=%s WHERE id=%s",
    ...
    if not vendas_rows:
        ...
        if df_f.empty:
            ...
            if not df_aberto.empty:
                ...
                df_ab = df_aberto.copy()
                df_ab.columns = ["Data","Cliente","Valor (R$)"]
...
# ══════════════════════════════════════════════
    ...
    def _carregar_clientes_db():
        return fetch("""
            SELECT id, nome, nome_completo, telefone, cpf, cep, logradouro,
    ...
    def _form_cliente(dados_iniciais: dict, label_btn: str, id_edicao=None):
        ...
        with st.container(border=True):
            ...
            cpf_v = fd2.text_input("🪪 CPF (para envio pelos Correios)", value=dados_iniciais.get("cpf","") or "", placeholder="000.000.000-00", key=f"form_cpf_{id_edicao}", help="O CPF é exigido pelos Correios para envio de encomendas.")
                ...
                if not nome_v.strip():
                    ...
                    if id_edicao == "NOVO":
                        ...
                        run("""UPDATE clientes SET
                            nome=%s, nome_completo=%s, telefone=%s, cpf=%s, cep=%s, logradouro=%s,
                            numero=%s, complemento=%s, bairro=%s, cidade=%s, estado=%s, observacoes=%s
                            WHERE id=%s""",
                            (nome_v.strip().lower(), nome_completo_v.strip(), tel_v, cpf_v.strip(),
                             cep_v, logradouro_v, numero_v, complemento_v, bairro_v, cidade_v,
